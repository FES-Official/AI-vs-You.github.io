document.addEventListener("DOMContentLoaded", () => {
  ...
  function prepareShuffledMemorySet() {
    shuffledMemorySet = [
      ...
      {
        description:
          "Shrinking Number Spiral: Track number in shrinking spiral.",
        pattern: [9, 7, 5, 3, 1],
      },
      {
  ...
  const impossibleChallenges = [
    {
      ...
      description:
        "A candidate answers confidently, but a 0.2s micro-expression shows fear. Why? What was the glitch?",
      expected: ["fear", "micro", "contradiction", "leak"],
    },
    {
      title: "The Refusal Gesture",
      description:
        "A character uses a gesture that seems polite. But in a certain culture, it's offensive. Decode it.",
      expected: ["gesture", "culture", "offensive", "context"],
    },
    {
      title: "The Laugh at the Funeral",
      description:
        "Someone laughs at a funeral. Nervousness? Trauma? Culture? Explain the paradox.",
  ...
  function startImpossibleChallenge(index) {
    ...
    el.output.innerHTML = `🚨 Impossible Challenge ${index + 1}: ${
      chal.title
