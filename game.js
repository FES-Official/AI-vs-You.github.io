...
function prepareShuffledMemorySet() {
  ...
  shuffledMemorySet = originalSet; // No shuffle as per request
}
  ...
  setTimeout(() => {
    ...
    const checkMemory = function (e) {
      if (e.key === "Enter") {
        ...
        const input = el.memoryInputField.value.trim().toLowerCase();
        el.memoryInputField.removeEventListener("keydown", checkMemory);
        el.memoryInputField.value = "";
        el.memoryInputField.classList.add("hidden");
        el.memoryChallenge.classList.add("hidden");
...
function startNextChallenge() {
  const hardQuestions = [
    ...
    {
      text: "AI: Four cards are on the table. Each has a number on one side and a letter on the other. You see: A, D, 4, 7. Rule: “If a card has a vowel on one side, it must have an even number on the other.” Which cards do you need to flip to test the rule?",
      answer: "A and 7",
    },
    {
      text: "AI: Three statements about a locked vault: 1. If the vault is not locked, then the alarm is on. 2. If the alarm is on, the guard is awake. 3. The guard is asleep. Is the vault locked?",
      answer: "YES",
    },
    {
      text: "AI: There are three boxes: - One contains only true statements. - One only false. - One mixed. Labels: A: B has only false. B: I am all true. C: A is mixed. Only one label is true. Which has mixed?",
      answer: "A",
    },
  ];
